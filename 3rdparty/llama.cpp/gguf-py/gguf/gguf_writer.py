...
class GGUFWriter:
    ...
    def add_rope_scaling_longrope2_factors(self, factors: Sequence[float]) -> None:
        self.add_array(Keys.Rope.SCALING_LONGROPE2_FACTORS.format(arch=self.arch),
                       [np.float32(f) for f in factors])

    def add_rope_scaling_longrope2_mscale_policy(self, policy: str) -> None:
        self.add_string(Keys.Rope.SCALING_LONGROPE2_MSCALE.format(arch=self.arch), policy)
