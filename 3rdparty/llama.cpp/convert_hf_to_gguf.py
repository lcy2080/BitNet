...
class Phi3MiniModel(Model):
    ...
    def set_gguf_parameters(self):
        ...
        elif rope_scaling_type == 'longrope2':
            attn_factor = math.sqrt(1 + math.log(scale) / math.log(orig_max_pos_embds)) if scale > 1.0 else 1.0
            # LongRoPE2 uses per-dimension rescaling factors
            longrope2_factors = rope_scaling.get('longrope2_factors', None)
            longrope2_mscale = rope_scaling.get('longrope2_mscale_policy', 'default')
            if longrope2_factors is not None:
                self.gguf_writer.add_rope_scaling_longrope2_factors(longrope2_factors)
                self.gguf_writer.add_rope_scaling_longrope2_mscale_policy(longrope2_mscale)
